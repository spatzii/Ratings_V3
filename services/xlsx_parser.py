...
class XlsxParser:
    """Parse a ratings Excel file and return normalized data.
...
# ... existing code ...
    ...
    async def process_ratings_file(self) -> dict:
        ...
        return XlsxParser.clean_data(xlsx_ratings_sheet, self.date)
    ...
    def clean_data(dataframe: pd.DataFrame, date: str) -> dict:
        ...
        dataframe.index = pd.to_datetime([
            XlsxParser.fix_broadcast_time(idx, date) for idx in dataframe.index
        ])
